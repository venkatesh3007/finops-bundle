...
// A sensible starter chart for a fresh workspace. Accounts are only created in
// the ledger when a row actually posts to them, so this costs nothing until used.
export const DEFAULT_CHART = [
  "Assets:Cash", "Assets:Clearing:SelfTransfers",
  "Income:Salary", "Income:Interest", "Income:Refunds", "Income:Reimbursement", "Income:Other",
  "Expenses:Rent", "Expenses:Groceries", "Expenses:Dining", "Expenses:Utilities", "Expenses:Subscriptions",
  "Expenses:Travel:Cabs", "Expenses:Travel:Flights", "Expenses:Travel:Hotels", "Expenses:Travel:Trains", "Expenses:Fuel",
  "Expenses:Health", "Expenses:Shopping", "Expenses:Education", "Expenses:Insurance", "Expenses:EMI", "Expenses:Interest",
  "Expenses:Fees", "Expenses:Entertainment", "Expenses:Gifts", "Expenses:Taxes", "Expenses:Other",
];
...
export async function classificationContext(entity) {
  ...
  let accounts = (await query("select name from accounts where entity_id=$1 and close_date is null order by name", [entId])).map((r) => r.name);
  // an empty/new book has nothing to classify INTO — offer the starter chart
  if (!accounts.some((a) => /^(Expenses|Income):/.test(a))) accounts = [...new Set([...accounts, ...DEFAULT_CHART])].sort();
  const decisions = {};
  ...
  // LEARNING FROM THE LEDGER IS OFF BY DEFAULT.
  //
  // This taught, for each payee, the account it most often posts to — an
  // invisible third classifier sitting behind the explicit rules, and the one
  // that survived every rule purge. It classified 900 of 5,617 transactions, and
  // it keyed on whatever derivePayee returned, which used to mean 1,198
  // transactions pooling under "UPIOUT" and teaching each other Expenses:EMI.
  //
  // Classification is now the model reading the row, plus rules a person added
  // deliberately. Set CLASSIFY_LEARN_FROM_HISTORY=1 to put it back.
  const learnFromHistory = process.env.CLASSIFY_LEARN_FROM_HISTORY === "1";
  const hist = !learnFromHistory ? [] : await query(
...
export async function importStatement(entity, { filename, sha256, bytes, account, kind, model, rows, opening_balance = null, force = false, userEmail }) {
  ...
  try {
    ...
    // IDEMPOTENT RE-IMPORT, keyed on the ROW, not on what the row says.
    //
    // This used to key on (date | amount | first 24 chars of description), which
    // cannot tell a re-import from a genuine repeat. Amex prints three separate
    // "PAYMENT RECEIVED. THANK YOU" of 100,000.00 on one day; IDBI repeats a UPI
    // amount to the same payee. All collapse to one key, so every repeat after
    // the first was silently discarded — 128 real rows across this book, worth
    // 1,149,065.74 on one card alone, and the account balance then disagreed with
    // the closing balance the statement prints.
    //
    // A row of a given statement is one transaction. Re-importing the same file
    // finds the same row ids and skips them; two distinct rows never collide,
    // however identical they look.
    const ex = await client.query(
      `select t.metadata->>'row' as row from transactions t
         join postings p on p.transaction_id = t.id
        where p.account_id=$1 and t.source_file=$2 and t.metadata ? 'row'`, [stmtAcct, path]);
    const existing = new Set(ex.rows.map((r) => String(r.row)));
    const any = await client.query("select 1 from postings where account_id=$1 limit 1", [stmtAcct]);
    const hadPostings = any.rows.length > 0;
    ...
    // A bank statement carries a running balance per row, so the opening is the
    // first row's balance backed out by its own amount. A CARD statement carries
    // none — balRows is empty — and without this fallback the account started at
    // zero and never matched the closing balance the statement prints.
    if (!hadPostings && (balRows.length || opening_balance != null)) {
      const f = balRows[0];
      const opening = balRows.length
        ? r2(balSign * Number(f.balance) - Number(f.amount))
        : r2(balSign * Number(opening_balance));
      if (Math.abs(opening) >= 0.01) {
        const seedFrom = balRows.length ? f.date : sorted[0].date;
        const d = new Date(seedFrom + "T00:00:00Z"); d.setUTCDate(d.getUTCDate() - 1);
    ...
    for (const r of sorted) {
      const key = String(r.i ?? rowKey(r));
      if (existing.has(key)) { skipped++; continue; }
      existing.add(key);
      const amt = r2(r.amount);
      const flag = r.flag === "!" ? "!" : "*";
      if (flag === "!") flagged++;
      const meta = { file: path, row: key, classified_by: r.source || null, rule: r.rule || null, confidence: r.confidence ?? null, imported_by: userEmail || null };
