...
export async function computePack(entity, todayISO) {
  ...
  const totalPeak = loans.reduce((s, l) => s + Math.abs(Number(l.peak)), 0) || 1;
  const totalNow = loans.reduce((s, l) => s + Math.abs(Number(l.now)), 0);
  const packTotals = {
    ...
    clearedPct: Math.max(0, Math.min(100, Math.round((1 - totalNow / totalPeak) * 100))),
  };
