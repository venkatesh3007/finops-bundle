...
// A sensible starter chart so the account picker is never empty — a brand-new
// (empty) workspace has no accounts of its own yet, so seed common categories.
export const DEFAULT_CANDIDATES = [
  "Income:Salary", "Income:Interest", "Income:Refund", "Income:Other",
  "Expenses:Food", "Expenses:Groceries", "Expenses:Rent", "Expenses:Utilities",
  "Expenses:Transport", "Expenses:Shopping", "Expenses:Health", "Expenses:Entertainment",
  "Expenses:Travel", "Expenses:Subscriptions", "Expenses:Fees", "Expenses:Education",
  "Expenses:Insurance", "Expenses:Other",
  "Assets:Cash", "Assets:Transfer", "Assets:Investments", "Assets:Receivable",
  "Liabilities:Loan", "Liabilities:CreditCard",
];
...
export function buildContext(raw) {
  ...
  const own = accounts.filter((a) => /^(Expenses|Income):/.test(a) || /^Assets:(Cash|Clearing|Receivable|Investments)/.test(a) || /^Liabilities:Loan/.test(a));
  // own accounts first (they carry the user's real taxonomy), then defaults, deduped —
  // so the picker always offers something even before the model loads / on an empty book.
  const candidates = [...new Set([...own, ...DEFAULT_CANDIDATES])];
...
// Accounts the MODEL must never auto-assign — they're deliberate placements
// (work fronting, holdings, debt), set by an explicit rule/decision in
// classifyByRules, not guessed from a description. Leaving them in the model's
// option list made a small model collapse everything into "Assets:Receivable"
// (esp. when a row's direction is ambiguous). They still appear in the manual
// picker and can be reached by rules.
const MODEL_ONLY_EXCLUDE = /^(Assets:Receivable|Assets:Investments|Liabilities:)/;

export function candidatesFor(row, ctx) {
  const inflow = row.amount > 0;
  const base = ctx.candidates.filter((a) => !MODEL_ONLY_EXCLUDE.test(a));
  const list = base.filter((a) => (inflow ? !a.startsWith("Expenses:") : !a.startsWith("Income:")));
  // Always keep a same-direction "Other" so a mis-signed row lands somewhere sane
  // rather than being forced onto an unrelated account.
  const other = inflow ? "Income:Other" : "Expenses:Other";
  if (!list.includes(other) && base.includes(other)) list.push(other);
