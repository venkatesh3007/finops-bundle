...
// The SIGN of the gap says which bug it is, and saying it wrongly actively
// misdirects a repair: a statement whose rows exceed its own printed total was
// told "8290 of charges is missing", so the repair round went looking for more
// charges while the cross-check was telling it to remove a duplicate. Two
// contradictory instructions, and nothing got fixed.
//   printed > extracted  → rows are MISSING
//   printed < extracted  → something is counted TWICE
function sideWord(gap, noun) {
  return gap > 0
    ? `${r2(gap)} of ${noun} has not been read`
    : `the rows total ${r2(-gap)} MORE than the statement says — something is counted twice`;
}
  ...
  return {
    ...
    note: !anyCheck ? "no running balance, opening/closing or printed totals to verify against — rows can't be auto-checked"
      ...
      : sides.credits && !sides.credits.ok
        ? `money IN doesn't match the statement's own total: it prints ${sides.credits.printed}, the rows add to ${sides.credits.extracted} — ${sideWord(sides.credits.gap, "credits/payments")}`
      : sides.debits && !sides.debits.ok
        ? `money OUT doesn't match the statement's own total: it prints ${sides.debits.printed}, the rows add to ${sides.debits.extracted} — ${sideWord(sides.debits.gap, "charges")}`
