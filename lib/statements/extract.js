...
async function callModel({ text, filename, bank, period, isChunk, chunkIndex, chunkTotal, priorBalance, rules = "" }) {
  const hints = [
...
export async function extractStatement({ pages, text, filename = "", bank = "", period = null, rules = "" } = {}) {
  ...
  for (let ci = 0; ci < chunks.length; ci++) {
    const out = await callModel({
      ...
      isChunk: chunks.length > 1, chunkIndex: ci, chunkTotal: chunks.length, priorBalance, rules,
    });
    if (ci === 0) {
