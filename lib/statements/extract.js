...
export async function extractStatement({ entity, pages, text, filename = "", bank = "", period = null, rules = "", hint = "", onNote = null } = {}) {
  ...
  try {
    const out = await extractWithModule({ source: version.source, pages, text, filename, bank, period, rules, hint, onNote });
    ...
    const out = await extractWithModule({ source: BASELINE_SOURCE, pages, text, filename, bank, period, rules, hint, onNote });
