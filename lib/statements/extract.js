...
export async function extractStatement({ entity, pages, text, filename = "", bank = "", period = null, rules = "", hint = "" } = {}) {
  ...
  try {
    const out = await extractWithModule({ source: version.source, pages, text, filename, bank, period, rules, hint });
    ...
    const out = await extractWithModule({ source: BASELINE_SOURCE, pages, text, filename, bank, period, rules, hint });
