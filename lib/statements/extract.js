...
const DEFAULT_MODEL = process.env.EXTRACT_MODEL || "claude-sonnet-5";
...
async function callModel({ apiKey, model, text, filename, bank, period, isChunk, chunkIndex, chunkTotal, priorBalance }) {
  const hints = [
  ...
  const res = await fetch(API, {
    ...
    body: JSON.stringify({ model, max_tokens: 16000, system: EXTRACT_PROMPT, messages: [{ role: "user", content: user }] }),
  });
...
export async function extractStatement({ pages, text, filename = "", bank = "", period = null } = {}) {
  ...
  if (!apiKey) return { error: "extract_not_configured" };
  ...
  for (let ci = 0; ci < chunks.length; ci++) {
    ...
    const out = await callModel({
      apiKey, model, text: body, filename, bank, period,
  ...
  return { ...meta, model, chunks: chunks.length, transactions: all, reconciliation: rec };
}
