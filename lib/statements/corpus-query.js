...
// Ranking statements — "which is the biggest / worst / oldest" is a sort, and
// without one the op just returned whatever happened to be first.
export const STATEMENT_SORTS = {
  rows: (s) => s.rows, breaks: (s) => s.breaks, inflow: (s) => s.inflow,
  outflow: (s) => s.outflow, net: (s) => s.net,
  from: (s) => s.from || "", to: (s) => s.to || "", name: (s) => s.name,
};

export function sortStatements(list, q = {}) {
  const key = STATEMENT_SORTS[q.sort];
  if (!key) return list;
...
export function runQuery(corpus, q = {}) {
  ...
  if (op === "statements") {
    const sel = sortStatements(filterStatements(statements, q), q);
    return { op, matched: sel.length, sorted_by: q.sort || null, order: q.order || "desc",
      result: sel.slice(0, limit).map((s) => ({
