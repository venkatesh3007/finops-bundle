...
export function crossCheck({ pages = [], rows = [], printed = {} } = {}) {
  ...
  if (sourceComplete === false) {
    return {
      ...
      source_complete: false, wrong_amount: [], missing: [], summary_lines_counted: [], duplicates, corrections: [],
  ...
  // APPLICABLE CORRECTIONS. When the cross-check can name both the offending row
  // and the exact edit, proposing it beats re-asking a model: the edit is derived
  // from the document, and the reconciler can PROVE it — if applying it makes the
  // statement tie to the rupee, it was right. Nothing is applied here; the caller
  // applies and re-verifies, and discards anything that doesn't improve.
  const corrections = [];
  for (const w of wrongAmount) {
    corrections.push({
      kind: "fix_amount", date: w.date, desc: w.desc, from: w.took, to: w.home,
      why: `"${w.desc}" is a foreign-currency line printing ${w.foreign} and ${w.home}; the home-currency amount is ${w.home}.`,
    });
  }
  for (const c of counted) {
    corrections.push({
      kind: "drop_row", amount: c.home, desc: c.desc,
      why: `"${c.desc}" ${c.home} sits under "${c.under_summary}" — a restatement of a transaction already counted.`,
    });
  }
  if (overBy) {
    for (const d of duplicates.filter((x) => near(x.amount, overBy))) {
      if (corrections.some((c) => c.kind === "drop_row" && near(c.amount, d.amount))) continue;
      corrections.push({
        kind: "drop_row", amount: d.amount, date: d.date, desc: d.desc,
        why: `${d.date} ${d.amount} appears twice and is exactly the ${overBy} this statement is over-counted by.`,
      });
    }
  }
