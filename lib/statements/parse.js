...
// The COUNTERPARTY, not the transaction type.
//
// The old pattern only stripped "UPI/", "NEFT/", "IMPS/", "RTGS/" — so
// "UPIOUT/649.../kamathmandar13@okicici" fell through to the text before the
// first slash and the payee became the literal string "UPIOUT". 1,198 unrelated
// transactions across 22 accounts pooled under that one label, and because
// classification learns per payee, they then taught each other. "UPI IN" pooled
// 231, "NFT" 125, "TO ECM" 69.
//
// Read it the way the line is actually built: skip the transaction-type token,
// then take the first segment that is not a reference number — which is the VPA
// or the name.
const TXN_KIND = /\b(?:UPI\s?IN|UPI\s?OUT|UPIOUT|UPIIN|UPI|NEFT|NFT|IMPS|RTGS|RTG|ACH|MMT|TFR)\b[\/\-\s]*/i;

// A bank reference, not a counterparty: pure digits, a dense alphanumeric code,
// or a one-token marker. An "@" means a VPA, which is always a counterparty.
const isRef = (t) => {
  const s = String(t).trim();
  if (!s || s.length <= 2) return true;
  if (/^\d[\d\s]*$/.test(s)) return true;
  if (/^(U|UPI|TFR|S\d+)$/i.test(s)) return true;
  const digits = (s.match(/\d/g) || []).length;
  return digits >= 4 && digits / s.length > 0.35 && !s.includes("@");
};

export function derivePayee(desc) {
  const d = String(desc || "").trim();
  if (!d) return "Statement";
  const m = d.match(TXN_KIND);
  if (m) {
    const rest = d.slice(m.index + m[0].length);
    // Split on "/" first; a hyphen only separates when there is no slash,
    // otherwise it tears VPAs apart (venkatesh3007-2@okaxis, crew-1251.payu@indus).
    const parts = rest.includes("/") ? rest.split("/") : rest.split("-");
    const seg = parts.map((x) => x.trim()).filter(Boolean).find((x) => !isRef(x));
    if (seg) return seg.split(/\s{2,}/)[0].trim().slice(0, 60);
  }
  const first = d.split(/[,\/]/)[0].trim();
  return (isRef(first) ? d : first || d).slice(0, 60);
}
