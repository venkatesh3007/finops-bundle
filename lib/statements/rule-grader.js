...
export async function gradeProposedRule(entity, ruleId, { onNote = null } = {}) {
  ...
  // Pass onNote DOWN. Without it a statement read is a single silent step: the
  // run looks stalled, and — because the reaper judges liveness by updated_at —
  // a slow-but-healthy read was being marked failed. Chunk notes give both the
