...
// NATIVE TOOL CALLING.
//
// chatText above returns a string, which is all the extractor needs. An agent
// needs the whole assistant turn: the text, the tool calls it wants to make, and
// the thinking blocks that have to be handed back next turn.
//
// Everything a hand-rolled protocol had to guess at is typed here. There is no
// "reply with ONE JSON object", no brace scanning, no deciding whether a preamble
// is prose or a plan, and no way for an empty completion to be mistaken for an
// answer — `tool_calls` is either present or it isn't.
export async function chatMessage(messages, { tools = [], max_tokens = 8000, effort = null, temperature = 0 } = {}) {
  if (!gatewayConfigured()) throw new Error("extract_not_configured");
  const body = { model: MODEL || "default", max_tokens, stream: true, messages };
  if (tools.length) body.tools = tools;
  if (effort) { body.thinking = { type: "adaptive" }; body.output_config = { effort }; }
  ...
  try {
    if (!(res.headers.get("content-type") || "").includes("text/event-stream")) {
      ...
      return { content: m.content || "", tool_calls: m.tool_calls || [], reasoning_blocks: m.reasoning_blocks || [], finish_reason: d?.choices?.[0]?.finish_reason || "stop" };
    }
    return await readSseMessage(res, ac);
  } finally { clearTimeout(hardStop); }
}

async function readSseMessage(res, ac) {
  const reader = res.body.getReader();
  const decoder = new TextDecoder();
  let buf = "", content = "", tool_calls = [], reasoning = [], finish = "stop", failure = null, done = false;
  let stall = setTimeout(() => ac?.abort(new Error("stalled")), STALL_MS);
  const beat = () => { clearTimeout(stall); stall = setTimeout(() => ac?.abort(new Error("stalled")), STALL_MS); };
  try {
    while (!done) {
      ...
      while ((cut = buf.indexOf("\n\n")) !== -1) {
        ...
        for (const line of frame.split("\n")) {
          ...
          if (Array.isArray(d.tool_calls) && d.tool_calls.length) tool_calls = d.tool_calls;
          if (Array.isArray(d.reasoning_blocks) && d.reasoning_blocks.length) reasoning = d.reasoning_blocks;
  ...
  if (failure) throw new Error(`gateway: ${failure}`);
  return { content, tool_calls, reasoning_blocks: reasoning, finish_reason: finish };
}
