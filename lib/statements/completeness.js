...
export function ordering(rows) {
  ...
  if (dates.length < 2) return { ok: true, monotonic: true, out_of_order: 0, reversed: false };
  let back = 0, forward = 0;
  for (let i = 1; i < dates.length; i++) {
    const a = parse(dates[i - 1]), b = parse(dates[i]);
    if (b < a) back++; else if (b > a) forward++;
  }
  // A statement printed newest-first never steps FORWARD in time. The previous
  // test asked for a descent on EVERY step (`back === dates.length - 1`), which a
  // real statement cannot satisfy: consecutive rows routinely share a date, and
  // an equal step is neither forward nor back. idbi-79657_2026-08 has 26 back
  // steps and 60 equal ones, so the reverse-order branch never once fired on real
  // data — the condition was unsatisfiable rather than merely strict.
  const reversed = back > 0 && forward === 0;
  return {
    ok: back === 0, monotonic: back === 0, out_of_order: back, reversed,
    note: back === 0 ? "" : reversed
      ? "rows are in reverse date order — the statement lists newest first"
      : `${back} row(s) are dated before the row above them, so these are not in statement order`,
  };
}
