...
const SCHEMA = `{
  ...
  "sort": "rows" | "breaks" | "inflow" | "outflow" | "net" | "from" | "to" | "name",  // op=statements: RANK them
  "order": "desc" | "asc",
...
"which is the biggest statement" -> {"op":"statements","sort":"rows","order":"desc","limit":1}
"which statement has the most balance breaks" -> {"op":"statements","sort":"breaks","order":"desc","limit":1}
"my oldest statement" -> {"op":"statements","sort":"from","order":"asc","limit":1}
"biggest statements" (plural) -> {"op":"statements","sort":"rows","order":"desc","limit":5}
ANY superlative about statements (biggest/largest/smallest/most/fewest/worst/oldest/newest) MUST set "sort".
...
export async function plan(question, corpus) {
  ...
  if (["rows", "breaks", "inflow", "outflow", "net", "from", "to", "name"].includes(q.sort)) out.sort = q.sort;
  ...
  // A superlative that arrives without a sort would return an arbitrary row and
  // read as "I can't tell" — infer the ranking from the wording instead.
  if (out.op === "statements" && !out.sort) {
    const t = question.toLowerCase();
    const asc = /\b(smallest|fewest|least|shortest|oldest|earliest)\b/.test(t);
    const superlative = asc || /\b(biggest|largest|most|worst|longest|newest|latest|top)\b/.test(t);
    if (superlative) {
      out.sort = /\bbreak|broken|wrong|worst\b/.test(t) ? "breaks"
        : /\bold|new|recent|earl|late\b/.test(t) ? "from"
        : /\bspen|out|debit\b/.test(t) ? "outflow"
        : /\bin|credit|receiv\b/.test(t) && !/\brow/.test(t) ? "inflow"
        : "rows";
      out.order = asc ? "asc" : "desc";
      if (!out.limit && /\b(the|which|what)\b/.test(t)) out.limit = 5;
    }
  }
...
export function render(q, res, corpus) {
  ...
  switch (res.op) {
    ...
    case "statements": {
      ...
      const LABEL = { rows: "rows", breaks: "balance breaks", inflow: "money in", outflow: "money out", net: "net", from: "start date", to: "end date", name: "name" };
      const head = res.sorted_by
        ? `Ranked by ${LABEL[res.sorted_by]} (${res.order === "asc" ? "lowest" : "highest"} first) — ${r[0].name} leads with ${
            res.sorted_by === "rows" ? `${r[0].rows} rows`
            : res.sorted_by === "breaks" ? `${r[0].breaks} balance breaks`
            : res.sorted_by === "from" ? r[0].from
            : res.sorted_by === "to" ? r[0].to
            : inr(r[0][res.sorted_by])}. Showing ${r.length} of ${res.matched}:`
        : `${plural(res.matched, "statement")}:`;
      return `${head}\n` + r.map((s) =>
