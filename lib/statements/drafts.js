...
export async function createAndProcess(entity, input) {
  ...
  const ins = await query(
    ...
    [entId, String(input.filename || "statement").slice(0, 200), input.sha256 || null, input.bytes || null, input.source || null, input.account || null, input.kind || null, JSON.stringify(meta), input.skip_parse ? "queued" : "processing"]);
