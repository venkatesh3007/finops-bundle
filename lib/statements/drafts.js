...
export async function importDraft(entity, id, { force = false, userEmail = null } = {}) {
  ...
  const rows = (d.rows || [])
    ...
    .map(({ date, desc, amount, balance, payee, account, source, rule, confidence, flag, sign_flipped }) => ({ date, desc, amount, balance, payee, account, source, rule, confidence, flag, sign_flipped }));
