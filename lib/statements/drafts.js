...
export async function importDraft(entity, id, { force = false, userEmail = null } = {}) {
  ...
  if (result.ok) {
    // NO SILENT RULE-LEARNING. This used to write a payee rule for every manually
    // categorised row. A rule changes how EVERY future statement classifies, so
    // learning one as a side effect of a bulk pass means a single addressing slip
    // becomes permanent state: when row assignments landed one row early, the
    // importer learned ten payees against their neighbour's account — SendGrid,
    // DigitalOcean and Uber One as Expenses:EMI — and those re-fired on every
    // later classification, long after the rows themselves were corrected.
    //
    // Teaching a rule stays available and deliberate: set_payee_rule, or the
    // game's reclassify, where the person is looking at that one payee.
