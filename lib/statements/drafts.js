...
export async function createAndProcess(entity, input) {
  ...
  const meta = { pages: Array.isArray(input.pages) ? input.pages : null, raw_rows: Array.isArray(input.rows) ? input.rows : null, hints: [] };
  const ins = await query(
    `insert into statement_drafts (entity_id, filename, sha256, bytes, source, account, kind, status, meta)
     values ($1,$2,$3,$4,$5,$6,$7,'processing',$8) returning *`,
    [entId, String(input.filename || "statement").slice(0, 200), input.sha256 || null, input.bytes || null, input.source || null, input.account || null, input.kind || null, JSON.stringify(meta)]);
  return processDraft(entity, ins[0].id, { hint: "" });
}
...
export async function processDraft(entity, id, { hint = "", remember = false } = {}) {
  await ensureDraftsTable();
  const entId = await entityId(entity);
  const r = await query("select * from statement_drafts where entity_id=$1 and id=$2", [entId, id]);
  ...
  try {
    ...
    if (meta.pages && meta.pages.length) {
      ...
      extracted = await extractStatement({ pages: meta.pages, filename: d.filename, bank: d.kind || "", rules, hint });
...
export async function importDraft(entity, id, { force = false, userEmail = null } = {}) {
  ...
  if (!d.account) throw new Error("set the statement account first");
