...
export async function warehouseMonth(entity, month) {
  ...
  for (const r of rows) {
    ...
    if (name.startsWith("Income:")) (FIXED_IN.test(name) ? Z.fixed_in : Z.var_in).push(shelf);
    else if (name.startsWith("Expenses:")) (fixedSet.has(name) ? Z.fixed_out : Z.var_out).push(shelf);
