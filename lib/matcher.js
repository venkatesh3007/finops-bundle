...
export async function matchMonth(entity, month) {
  ...
  // The counterparties rule: ≥₹10k unmatched is a real surprise (a card); anything
  // smaller is ordinary variable spend that AUTO-ABSORBS into the variable bucket —
  // never a card. This keeps exceptions at ~10-15/mo, never the raw pile.
  // A surprise is an unplanned PERSONAL expense or unexpected income ≥₹10k. Work
  // (reimbursable), investments, and loan movements are known/tracked — absorbed.
  const SURPRISE_MIN = 10_000;
  const isSurprise = (t) => t.mag >= SURPRISE_MIN && (t.kind === "expense" || t.kind === "income") && !t.adj;
