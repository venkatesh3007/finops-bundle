...
export async function matchMonth(entity, month) {
  ...
  const trows = await query(
    `select t.id, to_char(t.date,'YYYY-MM-DD') as date,
      ...
      -- A journal entry is not a line item. corrections were excluded; their
      -- UNDOs and SPLITs were not, so both halves of a reversal turned up as
      -- cards asking to be filed.
      where t.entity_id=$1 and to_char(t.date,'YYYY-MM')=$2 and t.corrects_id is null
        and not (t.metadata ? 'undo_of') and not (t.metadata ? 'split_of')
        and exists (
