...
import { resolveReclassify, splitReclassify, splitMany, undoLastFiling, markReview } from "./moves.js";
...
// Where a card can send its transaction. This returned ONLY Expenses:%, so an
// unexpected INFLOW could be filed as an expense but never as income, never as
// money someone owes you, and never as a transfer between people — the three
// things an inflow usually is. A reimbursement had nowhere to go.
//
// Now every account a movement can legitimately land on, and the card filters by
// direction: an inflow offers Income and the balance-sheet accounts, an outflow
// offers Expenses and the same balance-sheet accounts.
async function moveCategories(entId) {
  const rows = await query(
    `select name from accounts where entity_id=$1
       and (name like 'Income:%' or name like 'Expenses:%'
            or name like 'Assets:Receivable:%' or name like 'Assets:Transfers:%'
            or name like 'Assets:Investments:%' or name like 'Liabilities:Loans:%')
       order by name`, [entId]);
...
export async function monthState(entity, month) {
  ...
  return {
    ...
    // Plan is switched off, so there are no plan-miss cards — only reality.
    cards: [...surprises],
    deferred: deferred.map(shape),             // the "review later" stack
    streak: streakEndingAt(lockedMonths, locked ? month : prevMonth(month)),
    categories: exceptions ? await moveCategories(entId) : [],
  };
}
...
export async function resolveCard(entity, body) {
  ...
  // Put a crate on any shelf — a general reclassification from its current account.
  if (action === "recat") {
    const { txnId, fromAccount, toAccount, makeRule = false } = body;
    return await resolveReclassify(entity, { txnId, fromAccount, toAccount, makeRule });
  }

  // Split a surprise across two shelves (part here, part there).
  // put a crate back the way it was
  if (action === "undo") return await undoLastFiling(entity, { txnId: body.txnId });

  // split across several shelves in one decision
  if (action === "splitmany") {
    const { txnId, fromAccount, parts } = body;
    return await splitMany(entity, { txnId, fromAccount, parts });
  }

  if (action === "split") {
    const { txnId, fromAccount, toAccount, amount } = body;
    return await splitReclassify(entity, { txnId, fromAccount, toAccount, amount });
  }

  // Edit a planned line's amount (the plan was an estimate).
  if (action === "editplan") {
    const { planLineId, amount } = body;
    await query("update plan_lines set amount=$2 where id=$1 and entity_id=$3", [planLineId, Math.round(Number(amount)), entId]);
    return { planLineId, amount: Math.round(Number(amount)) };
  }
