...
export async function generateMoves(entity, { month } = {}) {
  ...
  const review = await query(
    `select distinct on (t.id) t.id, to_char(t.date,'YYYY-MM-DD') as date, t.payee,
       ...
       left join vettings v on v.transaction_id=t.id and v.status='ok'
      ...
      where t.entity_id=$1 and v.id is null and t.corrects_id is null
        and ($2::text is null or to_char(t.date,'YYYY-MM')=$2)
  ...
  // 3) MISSING COMMITMENT — a defined monthly commitment with no posting this month.
  let missing = [];
  if (month) {
    missing = await query(
