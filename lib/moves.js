  ...
  try {
    ...
    if (!(await monthSealed(client, entId, txnId))) {
      ...
      await client.query(
        ...
        [txnId, entId, toAccount, fromAccount]);
    ...
    // record the decision (the learned call)
    await client.query(
      `insert into decisions (entity_id, key, decision, rationale)
       values ($1,$2,$3,$4) on conflict (entity_id,key) do update set decision=excluded.decision`,
      [entId, `payee:${payee}`, toAccount, makeRule ? "rule: always this payee → this account" : "one-off reclassify"],
    );
    await client.query("commit");
...
export async function undoLastFiling(entity, { txnId }) {
  ...
  try {
    ...
    if (!(await monthSealed(client, entId, txnId))) {
      ...
      if (!stack.length) throw new Error("nothing to undo on this one — it has not been filed yet");
    ...
    const last = await client.query(
      `select t.id, to_char(t.date,'YYYY-MM-DD') date, t.payee, t.narration
        ...
        where t.entity_id=$1
          ...
          and coalesce((t.metadata->>'undone')::boolean,false) = false
    ...
    // mark the reversed entry so a second undo reaches the one before it
    await client.query(
      `update transactions set metadata = coalesce(metadata,'{}'::jsonb) || '{"undone":true}'::jsonb where id=$1`, [lastId]);
