...
async function callModel({ prompt, text, filename, bank, period, isChunk, chunkIndex, chunkTotal, priorBalance, rules, hint, effort = null }) {
  ...
  const raw = await chatText(
    ...
    { max_tokens: 16000, effort: effort || EXTRACT_EFFORT },
  );
...
// Reading someone's statement is worth thinking about. Override per deployment.
const EXTRACT_EFFORT = process.env.EXTRACT_EFFORT || "high";
...
export async function extractWithModule({ source, pages, text, filename = "", bank = "", period = null, rules = "", hint = "", onNote = null }) {
  ...
  const assess = () => {
    ...
    return { meta: m, droppedTotals, rows, rec, comp, cross };
  };
  ...
  for (let round = 1; round <= MAX_REPAIR_ROUNDS && !(best.rec.reconciled && best.comp.complete); round++) {
    ...
    // 3. never send the same request twice — escalate instead, then give up.
    const key = `${instruction}::${targets.join(",")}`;
    if (saidBefore.has(key)) {
      if (effort === "max") { await onNote?.("nothing further to try — leaving it flagged for review"); break; }
      effort = effort === "high" ? "max" : "high";
      await onNote?.(`same request as last time — thinking harder (effort ${effort}) instead of repeating it`);
    }
    saidBefore.add(key);
