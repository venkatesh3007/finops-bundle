...
export async function extractWithModule({ source, pages, text, filename = "", bank = "", period = null, rules = "", hint = "" }) {
  ...
  const rec = reconcile(rows, { statement_type: meta.statement_type, opening_balance: meta.opening_balance, closing_balance: meta.closing_balance });
  return { ...meta, model: gatewayModelLabel(), chunks: chunks.length, transactions: rows, reconciliation: rec };
}
