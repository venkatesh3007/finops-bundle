...
import { completeness, ordering } from "../statements/completeness.js";
...
export function isTransient(err) {
  ...
  return /gateway\s+(?:408|409|425|429|5\d\d)\b/i.test(m) ||
    ...
    /went quiet|no reply|exceeded the time limit|stalled/i.test(m);
}
...
export async function extractWithModule({ source, pages, text, filename = "", bank = "", period = null, rules = "", hint = "", onNote = null, repair = true }) {
  ...
  // WHERE IN THE DOCUMENT ARE WE. A statement read is one job step, but the work
  // inside it is one gateway call per chunk and can run for minutes — 2026-08
  // took thirteen of them and said nothing, because notes only fired on retries
  // and truncation. Approximate line spans (mod.chunk may drop or overlap text)
  // are still enough to answer "which part is it on".
  const spans = [];
  { let ln = 1;
    for (const c of chunks) { const n = String(c).split("\n").length; spans.push([ln, ln + n - 1]); ln += n; } }
  ...
  const runOne = async (ci, extra = "", effortOverride = null) => {
    ...
    await onNote?.(`part ${ci + 1} of ${chunks.length}${from ? ` (lines ${from}–${to})` : ""}…`);
  ...
  const assess = () => {
    ...
    const parsed = mod.postprocess(flat, { ...ctx, ...m });

    // NEWEST-FIRST IS NOT A PARSE ERROR. Some statements print the most recent
    // transaction at the top. Nothing is misread — but the continuity chain walks
    // rows in document order, so every single step looks like a broken balance.
    // idbi-79657_2026-08 scored 76 breaks this way, 94% of every break in the
    // corpus, and each one cost 1e4 in cost() — a 760,000 penalty that sent the
    // repair ladder hunting a fault that did not exist. Put the rows in the order
    // the balances run and the same extraction scores 2.
    const ord = ordering(parsed);
    const rows = ord.reversed ? [...parsed].reverse() : parsed;
    ...
    return { meta: m, droppedTotals, rows, rec, comp, cross, reversed: !!ord.reversed, counts: perChunk.map((r) => (r || []).length) };
  };
  ...
  return {
    ...
    // Recorded so the reordering is visible on the draft rather than silent — the
    // rows were re-sequenced, and an operator should be able to see that.
    rows_reversed: !!best.reversed,
