...
export async function activeVersion(entity, entId = null) {
  ...
  if (rows.length) {
    // v1 IS the code's baseline — it was seeded from BASELINE_SOURCE. If the code's
    // baseline has since improved, refresh the stored copy so the fix actually
    // reaches statements. A version the lab wrote has different notes and is never
    // touched here.
    const row = rows[0];
    if (row.version === 1 && row.notes === BASELINE_NOTES && row.source !== BASELINE_SOURCE) {
      const fresh = await query(
        "update extractor_versions set source=$2 where id=$1 returning *",
        [row.id, BASELINE_SOURCE]);
      return fresh[0] || row;
    }
    return row;
  }
