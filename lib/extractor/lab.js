...
export async function fixParser(entity, { complaint = "", dry_run = false } = {}) {
  ...
  const stale = items.filter((i) => i.version !== champion.version);
  const fresh = items.filter((i) => i.version === champion.version)
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
    ...
    if (v.promote) {
      if (dry_run) return { ok: true, dry_run: true, would_promote: true, diagnosis, attempts, champion_version: champion.version, corpus: { graded: items.length, total, problems } };
      ...
      return {
        ...
        improvements: v.improvements, corpus: { graded: items.length, total, problems },
  ...
  return {
    ...
    corpus: { graded: items.length, total, problems },
...
export async function checkParser(entity) {
  ...
  return { version: champion.version, scores: await evaluate(champion.source, items), corpus: { graded: items.length, total, problems } };
}
