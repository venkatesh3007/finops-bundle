...
export async function investigate(entity, history, { focus = null, onNote = null } = {}) {
  ...
  // The budget is on LOOKING, not on concluding. propose_rule is the conclusion,
  // not an investigation step, so it never costs a turn and is never refused —
  // the previous version forbade every tool on the last turn and so blocked the
  // one call that records the finding. That run ended, verbatim, on "I have
  // enough evidence to propose a rule. Let me summarize and propose."
  let looked = 0;
  for (let turn = 0; turn < MAX_TURNS + 4; turn++) {
    const spent = looked >= MAX_TURNS;
    if (spent) {
      messages.push({ role: "user", content: "That is all the investigation you get. Two things left: if you have PROVED a cause, call propose_rule now — it does not count against you. Otherwise write the answer from what you already established: what is wrong, the numbers you measured, and the cause. Do not ask for any other tool." });
    }
    ...
    if (spent && act.tool !== "propose_rule") {
      // It reached for more looking with nothing left. Its own words still beat a
      // raw JSON dump.
      return { reply: act.thought ? `${act.thought}\n\n${findings()}` : findings(), trace, proposed_rules: proposed, out_of_turns: true };
    }
    if (act.tool !== "propose_rule") looked++;
