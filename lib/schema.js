...
-- ── Extractor lab (the statement extractor's own, versioned source) ──────────
-- The extractor is a module (prompt + preprocess/chunk/postprocess) that the
-- system rewrites for itself when you report a parsing problem. Versions are
-- per-entity; fixtures are the real statements a candidate is graded on, scored
-- by the reconciler against each statement's own printed running balance.
...
create index if not exists extractor_versions_entity_idx on extractor_versions (entity_id, version desc);

create table if not exists extractor_fixtures (
  id          uuid primary key default gen_random_uuid(),
  entity_id   uuid not null references entities(id) on delete cascade,
  name        text not null,
  complaint   text,
...
create index if not exists extractor_fixtures_entity_idx on extractor_fixtures (entity_id, created_at desc);
