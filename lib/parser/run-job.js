...
import { priorResult, regressionReason } from "./gates.js";
...
export async function parseDraftWithCodegen({ entity, entId, jobId, draft, regenerate = false }) {
  ...
  const cachedRow = regenerate ? null : await getTemplate(entId, fp);
  const cached = cachedRow ? { source: cachedRow.source, label: cachedRow.label } : null;
  if (regenerate) await step(jobId, "note", `Writing a brand-new parser for this layout (ignoring the one I had cached).`);
  else if (!cached) await step(jobId, "note", `New layout (${fp}) — I haven't written a parser for this one yet.`);
  ...
  const rejected = regressionReason(prior, out.score);
  if (rejected) {
    await step(jobId, "note", `Rewrite rejected — ${rejected}. The stored parse is kept exactly as it was.`);
    const kept = {
      draft_id: draft.id, name: draft.filename, rows: prior.rows, breaks: prior.breaks,
      reconciled: prior.reconciled, rounds: out.rounds, reused: !!out.reused, fingerprint: fp, rejected,
    };
    await step(jobId, "done",
      `${draft.filename}: kept the previous parse — ${prior.rows} rows, ${prior.breaks} balance breaks${prior.reconciled ? " — reconciles ✓" : ""}.`, kept);
    return kept;
  }

  // Only a parser we could actually CHECK earns a place in the layout cache — a
  // cached parser is reused with no model call on every future statement of that
  // layout, so caching an unverified one propagates it silently. envelope_ok is
  // null (not false) when there was nothing to check, which is exactly how 13
  // unverified parsers previously got cached as good.
  const clean = out.score.rows > 0 && out.score.breaks === 0 && !out.score.zero_amounts &&
    out.score.envelope_ok !== false && out.score.verifiable === true;
  if (clean) {
...
export async function runParseJob({ entity, entId, jobId, draftIds, regenerate = false }) {
  const results = [];
  try {
    for (let i = 0; i < draftIds.length; i++) {
      ...
      try {
        results.push(await parseDraftWithCodegen({ entity, entId, jobId, draft: r[0], regenerate }));
      } catch (e) {
