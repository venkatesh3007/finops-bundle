...
export async function parseDraftWithCodegen({ entity, entId, jobId, draft }) {
  ...
  await checkCancel(jobId);
  if (out.score.rows > 0 && out.score.breaks === 0) {
  ...
  await step(jobId, "done", `${draft.filename}: ${out.score.rows} rows, ${out.score.breaks} balance breaks${out.score.reconciled ? " — reconciles ✓" : ""}.`, summary);
  return summary;
}
