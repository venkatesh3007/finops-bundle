...
// Is this parser actually good enough to accept? "No balance breaks" alone is
// NOT enough: rows with a null balance are skipped by the continuity check, so a
// parser that drops an amount can score zero breaks while losing money. The
// envelope (opening + net = closing) is what catches that, and a row with a zero
// amount is a parse failure on its face.
function acceptable(score) {
  ...
  // nothing verifiable at all (no balances, no opening/closing): the best we can
  // say is that it produced clean-looking rows — accept, but it stays unverified.
  return true;
}
...
function failureReport(parsed, rec) {
  ...
  if (!rec.verifiable) lines.push(`Nothing could be checked: ${rec.note}`);
...
export async function generateParser({ pages, filename = "", bank = "", cached = null, onStep = async () => {} }) {
  ...
  for (let round = 1; round <= MAX_ROUNDS; round++) {
    ...
    const cost = (x) => x.score.breaks + x.score.zero_amounts * 3 + (x.score.envelope_ok === false ? 5 : 0);
