...
function failureReport(parsed, rec) {
  ...
  const lines = [`${parsed.transactions.length} rows extracted, ${m.length} of them break the running balance.`];
  if (rec.envelope && !rec.envelope.ok) {
    lines.push(`Opening + net comes to ${rec.envelope.expected_closing} but the statement closes at ${rec.envelope.printed_closing} — that gap usually means rows are MISSING, not mis-read.`);
  }
...
export async function generateParser({ pages, filename = "", bank = "", cached = null, onStep = async () => {} }) {
  ...
  if (cached) {
    ...
    try {
      ...
      if (score.reconciled || score.breaks === 0) return { source: cached.source, parsed, rec, score, rounds: 0, reused: true };
  ...
  for (let round = 1; round <= MAX_ROUNDS; round++) {
    ...
    if (!best || score.breaks < best.score.breaks || (score.breaks === best.score.breaks && score.rows > best.score.rows)) best = candidate;
    if (score.reconciled || (score.breaks === 0 && score.rows > 0)) return candidate;
