...
export async function generateParser({ pages, filename = "", bank = "", cached = null, onStep = async () => {} }) {
  ...
  for (let round = 1; round <= MAX_ROUNDS; round++) {
    ...
    previous = source;
    if (round < MAX_ROUNDS) {
      // With an unverifiable parse `breaks` is 0, so the old wording read
      // "0 rows still don't chain" — say what actually went wrong instead.
      await onStep("note", !score.verifiable
        ? `Nothing in that parse could be checked — no row came back with a balance. Telling it to find the balance column and trying again.`
        : `${score.breaks} rows still don't chain — telling it exactly which ones and trying again.`);
    }
  }
