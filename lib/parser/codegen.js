...
import { acceptable, attemptCost, groundBalances } from "./gates.js";
...
function failureReport(parsed, rec) {
  ...
  if (parsed.dropped) lines.push(`${parsed.dropped} balance figure(s) you returned do not appear anywhere in the document, so they were discarded — they were computed, not read. Only ever return a balance that is printed on the page.`);
  if (zeros.length) {
  ...
  if (!rec.verifiable) {
    lines.push(`NOTHING COULD BE CHECKED: ${rec.note}. No row carried a balance and no opening/closing figure was returned, so there is no way to tell whether any amount is right. Most statements print a running balance column, an opening balance or a closing balance somewhere — if this one does, READ IT: copy the figure exactly as printed into \`balance\` on each row, or into \`opening_balance\`/\`closing_balance\`. NEVER COMPUTE A BALANCE. A running total you calculated from the amounts is not a balance — checking it proves nothing, because it is derived from the very numbers it is meant to check, and any balance that does not appear in the document is discarded anyway. If the statement genuinely prints no balance of any kind, return none and leave it unverified — that is the correct and expected outcome for such a statement.`);
  }
...
// Why this attempt was rejected, in words. Every rejection except a broken chain
// has breaks === 0, so wording this off `breaks` alone printed the nonsense
// "0 rows still don't chain" whenever the real problem was something else.
function rejectionNote(score) {
  if (!score.verifiable) return "Nothing in that parse could be checked — no row came back with a balance, and no opening or closing figure either. Telling it to find them.";
  if (score.zero_amounts) return `${score.zero_amounts} row(s) came out with no amount, which is where money goes missing. Telling it which ones.`;
  if (score.envelope_ok === false) return "The rows chain, but opening + net doesn't reach the printed closing balance — so a row is missing or an amount was misread. Telling it the gap.";
  return `${score.breaks} row(s) still don't chain — telling it exactly which ones.`;
}
...
export async function generateParser({ pages, filename = "", bank = "", cached = null, onStep = async () => {} }) {
  ...
  if (cached) {
    ...
    try {
      const parsed = groundBalances(compileParser(cached.source)(pages), pages.join("\n"));
  ...
  for (let round = 1; round <= MAX_ROUNDS; round++) {
    ...
    let source, run;
    try {
      source = cleanSource(await chatText(authorMessages({ sample, filename, bank, previous, failure }), { max_tokens: 4000, effort: process.env.CODEGEN_EFFORT || "high" }));
    ...
    let parsed;
    try {
      // Ground the balances before anything grades them: a balance the document
      // does not print was not read, it was invented (see gates.js).
      parsed = groundBalances(run(pages), pages.join("\n"));
    } catch (e) {
    ...
    previous = source;
    if (round < MAX_ROUNDS) await onStep("note", `${rejectionNote(score)} Trying again.`);
  }
